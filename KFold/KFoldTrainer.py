...
class Trainer():
  ...
  '''
  def FindBestWeight(self, Threshold, FoldName, ValAcc):
    assert self.fold_index_dict.shape[0] != 0, 'There isn\'t any database for search'

    for i in range(self.fold_index_dict.shape[0]):
      if
  '''
